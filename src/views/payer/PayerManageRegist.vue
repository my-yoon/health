<template>
    <section>
                        ...
                        <tbody>
                                    ...
                                    <div class="reg-group">
                                        ...
                                        <div class="reg-item">
                                            <button type="button" class="btn btn-slm"
                                                ...
                                                @click="modalOpen('isShowID', formData.admnId)">
                                                중복체크
                                            </button>
                                        </div>
                                    </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="reg-group">
                                        <div class="reg-item">
                                            <commselect :selectType="'searchCode.rprsTlnoFno'"
                                        ...
                                        <div class="reg-item">
                                            <input type="text" maxlength="4"
                                                ...
                                                :value="state.rprsTlnoMno" data-target="rprsTlnoMno" @input="setInputNumber"
                                                :disabled="state.brnCheck === 'Y'">
                                        </div>
                                        <span class="ui-coupler">-</span>
                                        <div class="reg-item">
                                            <input type="text" maxlength="4"
                                                :class="['form-control', { error: checkValidState('rprsTlnoMno') }]"
                                                :value="state.rprsTlnoBno" data-target="rprsTlnoBno" @input="setInputNumber"
                                                :disabled="state.brnCheck === 'Y'">
                                        </div>
                                    </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="reg-group">
                                        <div class="reg-item">
                                            <input type="text" placeholder="영문4자리의 기업코드를 입력하여 주세요"
                                                :class="['form-control', { error: checkValidState('ntprUcd') }]"
                                                v-model="formData.ntprUcd" :disabled="state.brnCheck === 'Y'">
                                        </div>
                                        <div class="reg-item">
                                            <button type="button" class="btn btn-slm"
                                                :disabled="formData.ntprUcd.length < 4 || state.brnInfo.ntprUcd === formData.ntprUcd || state.ccdCheck === 'N' || state.brnCheck === 'Y'"
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <div class="upload-file-box">
                                        ...
                                        <div class="upload-file-list" v-if="state.bizBanklist.length > 0">
                                            <div class="upload-file-list-item flex space-between"
        ...
        <div class="btn-bottom-set flex justify-center">
            <button type="button" class="btn btn-sl nega" @click="goToPage('/payer/regist/list')">취소</button>
            <button type="button" class="btn btn-sl nega" @click="resetFormData">초기화</button>
            <button type="button" class="btn btn-sl posi" @click="registPayer">저장</button>
        </div>
...
import { _duplicateCheck, _registJoinReqeustPayer, _duplicateEidCheck, _duplicateCcdCheck, _duplicateEmailCheck } from '@/api/payer.js';
...
export default {
    ...
    setup() {
        ...
        const { goToPage, validPassword, validEmail, getImageInfo } = useCommFunc();
        ...
        const state = reactive({
            ...
            brnType3: '',
            brnCheck: '',
            ntprSn: null, // 사업자Sn (중복체크 후 업데이트용)
            ccdCheck: '', // 기업코드 중복체크
            emadCheck: '', // 이메일 중복체크
            brnInfo: {},
        ...
        const formData = reactive({
            ...
            admnLvlId: 'B00' // 관리자레벨 Manager:B00,Associate:B01
        });
        ...
        const resetFormData = () => {
            ...
            formData.admnLvlId = 'B00'; // 관리자레벨 Manager:B00,Associate:B01
        };
        ...
        const modalOpen = async (modalName, params) => {
            ...
            if (modalName === 'isShowID') {
                ...
                if (state.brnCheck === 'Y' && state.membertype !== null) {
                    console.log('state.brnCheck', state.membertype.includes('111002'));
                    let joinType = '';
                    ...
                    if (state.membertype.includes('111002')) { // 페이어로 가입 되어 있을 때 ( 여기는 페이어 가입 페이지 )
                        msg = '페이어로 가입되어 있는 사업자등록번호 입니다. 동일 사업자등록번호로 중복가입이 불가 합니다.';
                    } else {
                ...
                $Modal.simple({
                    ...
                    .then(success => {
                        ...
                        if (state.brnCheck === 'Y') {
                            if (state.membertype !== null && state.membertype.includes('111002')) {
                                state.brnType1 = '';
                                state.brnType2 = '';
                                state.brnType3 = '';
        ...
        const setBrnSetting = () => {
            ...
            formData.bkcd = state.brnInfo.bkcd; // 은행
            state.acno = state.brnInfo.acno; // 법인계좌번호
            formData.accountNm = state.brnInfo.accountNm;
            formData.ntprUcd = state.brnInfo.ntprUcd; // 기업코드 >> 기업코드 중복체크 pass 필요
            // formData.rcmNtprUcd = state.brnInfo.rcmNtprUcd;
            formData.dlngPlcCd = state.brnInfo.dlngPlcCd; // 거래처코드(더존ERP)

            state.ccdCheck = 'N'; // 기업코드 중복체크 PASS
        };
         ...
         * 가입신청 등록
         */
        const registJoinReqeustPayer = async () => {
            try {
                let params = {
                ...
                const response = await _registJoinReqeustPayer(params);
                if (response.data.code === 'OK') {
         ...
         */
        const validFormData = () => {
            const target = ['admnId', 'admnPwd', 'checkAdmnPwd', 'ntprNm', 'brn', 'rprsvNm', 'bcoNm', 'indtpNm', 'rprsTlnoMno', 'adr', 'bkcd', 'acno', 'accountNm', 'ntprUcd', 'dlngPlcCd', 'admnDepNm', 'admnNm', 'admnHhpnoFno', 'admnHhpnoMno', 'admnHhpnoBno', 'admnEmad', 'bzlcCpAtflSn', 'ntprAcnCpAtflSn'];
            // const target = ['admnId'];
            ...
            for (const item of target) {
                ...
                // 예외 케이스 (비밀번호 규칙 오류)
                if (item === 'admnPwd' && !validPassword(formData[item])) {
