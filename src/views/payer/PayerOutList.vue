<template>
    <section>
        ...
        <div class="tbl-wrap">
            ...
            <NoData :nodatatext="'조회된 데이터가 없습니다.'" v-if="state.payerOutlist.length === 0"></NoData>
            <div v-else>
                <AgGridVue :columnDefs="state.tableColum_c" :rowData="state.payerOutlist"
...
import { _getPayerOutRequset } from '@/api/payer.js';
export default {
    ...
    setup() {
        ...
        const exelParams = {
            params: formData,
            url: '/common/api/v1/payer/payer-secession-request/excel'
        };
        ...
        const onCellClicked = (e) => {
            console.log(e.data);
            goToPage(`/payeroutlistdetail?Sn=${e.data.sn}`);
        };
        // 삭제할 컬럼 선택
        ...
        //리스트 조회
        const getPayerOutRequset = async () => {
            try {
                let params = {
                    size: pager.size,
                    offset: pager.offset,
                ...
                const response = await _getPayerOutRequset(params);
                console.log(response.data);
                state.payerOutlist = response.data.data.list;
