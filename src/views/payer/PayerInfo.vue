<template>
    <section>
        ...
        <div class="tbl-wrap">
            ...
            <NoData :nodatatext="'조회된 데이터가 없습니다.'" v-if="state.payerInfolist.length === 0"></NoData>
            <div v-else>
                <AgGridVue :columnDefs="state.tableColum_c" :rowData="state.payerInfolist"
...
import { _getPayerInfo } from '@/api/payer.js';
export default {
    ...
    setup() {
        ...
        const initColum = ref([
            ...
            // { headerName: '사업자 등록번호', field: 'brn', width: 350 },
            {
                headerName: '사업자 등록번호',
                field: 'brn',
                maxWidth: 350,
            ...
            { headerName: '가입일', field: 'admnJnDt', maxWidth: 150 },
            {
                headerName: '수정일',
                field: 'updDt',
                maxWidth: 150,
        ...
        const exelParams = {
            params: formData,
            url: '/common/api/v1/payer/payer-information/excel'
        };
        ...
        const getPayerInfo = async () => {
            try {
                ...
                const response = await _getPayerInfo(params);
                console.log(response.data);
                state.payerInfolist = response.data.data.list;
        ...
        const onCellClicked = (e) => {
            console.log(e.data);
            // goToPage(`/payerinfodetail?ntprSn=${e.data.sn}`);
        };
