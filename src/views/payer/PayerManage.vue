        ...
        <div class="tbl-wrap">
            <div class="table-util flex space-between">
                ...
                <div class="btn-set-m flex align-end">
                    ...
                    <selectBox :selectType="'page'" :initData="state.pagesize" @changedValue="selectedOptions_list" />
...
import { _getJoinRequestPayer } from '@/api/payer.js';
export default {
    ...
    setup() {
        ...
        const initColum = ref([
            ...
            // { headerName: '구분', field: 'ptnrSelrSvcTyNm', maxWidth: 100 },
        ...
        const exelParams = {
            params: formData,
            url: '/common/api/v1/payer/join-request/excel'
        };
        ...
        // 공통코드 셀렉트
        const selectedOptions = (valueCode, type) => {
            console.log('change select callback >>', type);
        ...
        const onCellClicked = (e) => {
            console.log(e.data);
            // goToPage(`/payermanagedetail?ntprSn=${e.data.sn}&mskgnRlsYn=${state.mskgnRlsYn}`);
            goToPage(`/payermanagedetail?admnSn=${e.data.sn}&ntprSn=${e.data.ntprSn}`);
        };
        ...
        // 리스트 조회
        const getJoinRequestPayer = async () => {
            try {
                let params = {
                    size: pager.size,
                    offset: pager.offset,
                    bgnDate: formData.inqType == '' ? '' : state.startDay,
                    endDate: formData.inqType == '' ? '' : state.endDay,
                ...
                console.log(params);
                const response = await _getJoinRequestPayer(params);
