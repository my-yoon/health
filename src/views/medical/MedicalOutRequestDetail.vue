<template>
    <section>
        ...
        <div class="btn-bottom-set flex justify-center">
            <button type="button" class="btn btn-sl nega" @click="goToPage('/medical/ban_request/list')">
                취소
            </button>
...
import { _getMedicalOutRequestDetail, _apprdMedical, _rejectMedical } from '@/api/medical.js';
...
export default {
    ...
    setup() {
        ...
        onMounted(() => {
            ...
            getMedicalOutRequestDetail(state.detailNum);
        ...
        */
        ...
        // 상세
        const getMedicalOutRequestDetail = async () => {
            try {
                let params = {
                    mskgnRlsYn: state.mskgnRlsYn ? 'Y' : 'N'
                };
                const response = await _getMedicalOutRequestDetail(state.detailNum, params);
        ...
        //승인
        const apprdMedical = async () => {
            try {
                const response = await _apprdMedical(state.detailNum);
                if (response.data.code === 'OK') {
                    await goToPage('/medical/ban_request/list');
                } else {
        ...
        //반려
        const rejectMedical = async () => {
            try {
                const response = await _rejectMedical(state.detailNum);
                if (response.data.code === 'OK') {
                    await goToPage('/medical/ban_request/list');
                } else {
                    console.log(response.data.message);
