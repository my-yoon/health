<template>
    <section>
        ...
        <div class="btn-bottom-set flex justify-center">
            <button type="button" class="btn btn-sl nega" @click="goToPage('/medical/info/list')">취소</button>
...
import { _getMedicalInfoDetail, _duplicateEidCheck } from '@/api/medical.js';
...
export default {
    ...
    setup() {
        ...
        //상세조회
        const getMedicalInfoDetail = async () => {
            try {
                let params = {
                    mskgnRlsYn: state.mskgnRlsYn ? 'Y' : 'N'
                };
                const response = await _getMedicalInfoDetail(state.detailNum, params);
        ...
        const registSellerInfoDetail = async () => {
            try {
                ...
                // await goToPage('/medical/info/list');
            } catch (error) {
                console.log(error);
            }
        };
        ...
        const modalOpen = async (modalName, params) => {
            ...
            if (modalName === 'isShowBrn') {
                ...
                if (state.brnCheck === 'Y' && state.membertype === '111004') {
                    msg = 'Seller로 가입되어 있는 사업자등록번호 입니다. 동일 사업자등록번호로 중복가입이 불가 합니다.';
                } else if (state.brnCheck === 'Y' && (state.membertype === '111005' || state.membertype === '111003' || state.membertype === '111002')) {
