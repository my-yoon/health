<template>
    <section>
        ...
        <div class="tbl-wrap">
            ...
            <NoData :nodatatext="'조회된 데이터가 없습니다.'" v-if="state.outRequestlist.length === 0"></NoData>
            <div v-else>
                <AgGridVue :columnDefs="state.tableColum_c" :rowData="state.outRequestlist"
...
import { _getMedicalOutRequest } from '@/api/medical.js';
export default {
    ...
    setup() {
        ...
        const initColum = ref([
            ...
            { headerName: '반려여부', field: 'admnJnReqDt', maxWidth: 120 },
            { headerName: '탈퇴신청일자', field: 'admnPtnrStNm', maxWidth: 90 }

        ]);
        const state = reactive({
            outRequestlist: [],
            outRequestlistTotal: 10,
        ...
        const exelParams = {
            params: formData,
            url: '/common/api/v1/medical/medical-secession-request/excel'
        };

        onMounted(() => {
            getMedicalOutRequest();
        ...
        const onCellClicked = (e) => {
            console.log(e.data);
            goToPage(`/medicaloutrequestdetail?Sn=${e.data.sn}`);
        };
        // 삭제할 컬럼 선택
        ...
        //리스트 조회
        const getMedicalOutRequest = async () => {
            try {
                let params = {
                    size: pager.size,
                    offset: pager.offset,
                ...
                console.log(params);
                const response = await _getMedicalOutRequest(params);
                console.log(response.data);
                state.outRequestlist = response.data.data.list;
