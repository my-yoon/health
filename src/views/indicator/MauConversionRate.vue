...
import { _getMauConversionRate } from '@/api/indicator.js';
...
export default {
    ...
    setup() {
        ...
        const onChangedPage = (num) => {
            ...
            //api호출
            getMauConversionRate();
        };

        //검색조건에 따른 리스트 재조회
        const reloadList = async () => {
            //차트도 초기화
            await getMauConversionRateChart();
        ...
        // 차트조회
        const getMauConversionRateChart = async () => {
            try {
                // 차트 조회
                formData.schTy = 'C';
                await _getMauConversionRate(formData).then(result => {
        ...
        //리스트 조회
        const getMauConversionRate = async () => {
            try {
                // 리스트 조회
                formData.schTy = 'L';
                await _getMauConversionRate(formData).then(result => {
        ...
        const periodSortGroupChange = () => {
            getMauConversionRateChart();
        };
        const excelParams = {
            params: formData,
            url: '/common/api/v1/statics/mau-conversion-rate/excel'
        };
