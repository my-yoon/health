...
import { _getServiceAccessUsage } from '@/api/indicator.js';
...
export default {
    ...
    setup() {
        ...
        const initColum = ref([
            ...
            {
                headerName: 'WAU(메인화면 방문)',
                flex: 1,
                field: 'man_cnt',
                valueFormatter: toLocaleStringKo
            },
            {
                headerName: 'WAU(라이프로그 기록)',
                flex: 1,
                field: 'lfl_cnt',
                valueFormatter: toLocaleStringKo
            },
            {
                headerName: 'WAU(챌린지 참여)',
                flex: 1,
                field: 'chl_cnt',
                valueFormatter: toLocaleStringKo
            }
        ...
        const state = reactive({
            ...
            chart: {
                ...
                options: {
                    ...
                    scales: {
                        y: {
                            title: { display: true, text: '건 수', rotate: 270 }
                        }
                    }
                }
            },
        ...
        const onChangedPage = (num) => {
            ...
            //api호출
            getServiceAccessUsage();
        };

        //검색조건에 따른 리스트 재조회
        const reloadList = async () => {
            //차트도 초기화
            await getServiceAccessUsageChart();
        ...
        // 차트조회
        const getServiceAccessUsageChart = async () => {
            try {
                // 차트 조회
                formData.schTy = 'C';
                await _getServiceAccessUsage(formData).then(result => {
        ...
        //리스트 조회
        const getServiceAccessUsage = async () => {
            try {
                // 리스트 조회
                formData.schTy = 'L';
                await _getServiceAccessUsage(formData).then(result => {
        ...
        const periodSortGroupChange = () => {
            getServiceAccessUsageChart();
        };

        const excelParams = {
            params: formData,
            url: '/common/api/v1/statics/service-access-usage/' + formData.sauTy + '/excel'
        };
