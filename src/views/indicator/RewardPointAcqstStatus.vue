...
import { _getRewardPointAcqstStatus } from '@/api/indicator.js';
...
export default {
    ...
    setup() {
        ...
        const onChangedPage = (num) => {
            ...
            //api호출
            getRewardPointAcqstStatus();
        };

        //검색조건에 따른 리스트 재조회
        const reloadList = async () => {
            //차트도 초기화
            await getRewardPointAcqstStatusChart();
            ...
            if (state.chartList.length > 0) {
                ...
                state.chart.datasets = Object.keys(state.chartList[0].yAxis[0]).filter(keys => !state.chart.datalabels[keys]?.disabled).map((item, index) => {
                    return {
                        ...
                        yAxisID: state.chart.datalabels[item]?.yAxisID ?? 'y1'
                    };
                });
        ...
        // 차트조회
        const getRewardPointAcqstStatusChart = async () => {
            try {
                // 차트 조회
                formData.schTy = 'C';
                await _getRewardPointAcqstStatus(formData).then(result => {
        ...
        //리스트 조회
        const getRewardPointAcqstStatus = async () => {
            try {
                // 리스트 조회
                formData.schTy = 'L';
                await _getRewardPointAcqstStatus(formData).then(result => {
        ...
        const periodSortGroupChange = () => {
            getRewardPointAcqstStatusChart();
        };

        const excelParams = {
            params: formData,
            url: '/common/api/v1/statics/reward-point-acqst-status/excel'
        };
        return {
            //공통 사용
            $Modal,
