...
import { _getAppVersion } from '@/api/indicator.js';
...
import { chartColorSet, toLocaleStringKo, toRound } from '@/views/indicator/util/chart.js';
...
export default {
    ...
    setup() {
        ...
        const defalutInitColum = [
            { headerName: 'App Version', width: 200, field: 'app_ver', pinned: 'left' }
        ];
        const initColum = ref([]);
        ...
        const chartInit = () => {
            if (state.chartList.length > 0) {
                ...
                state.chart.datasets = [{
                    label: '',
                    data: state.chartList.map(item => item.yAxis[0].mbr_cnt)
                }];
                //높이설정
        ...
        // 차트조회
        const getAppVersionChart = async () => {
            try {
                // 차트 조회
                formData.schTy = 'C';
                await _getAppVersion(formData).then(result => {
                ...
                await _getAppVersion(formData).then(result => {
                    ...
                    const _list = listAll.map((item, index) => {
                        ...
                        for (let i = keys.length - 1; i >= 0; i--) {
                            const key = keys[i] === 'app_ver' ? keys[i] : dayJS(keys[i]).format('YYYY-MM-DD');
        ...
        const excelParams = {
            params: formData,
            url: '/common/api/v1/statics/app-version/excel'
        };
        return {
            //공통 사용
            $Modal,
