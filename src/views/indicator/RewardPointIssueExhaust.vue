...
import { _getRewardPointIssueExhaust } from '@/api/indicator.js';
...
export default {
    ...
    setup() {
        ...
        const initColum = ref([
            ...
            {
                headerName: '총 지급완료 포인트',
                flex: 1,
                field: 'pnt_tot',
                valueFormatter: toLocaleStringKo
            },
            {
                headerName: '즉시 지급완료 포인트',
                flex: 1,
                field: 'pnt_imd',
                valueFormatter: toLocaleStringKo
            },
            {
                headerName: '수동 지급완료 포인트',
                flex: 1,
                field: 'pnt_mul',
                valueFormatter: toLocaleStringKo
            },
        ...
        const onChangedPage = (num) => {
            ...
            //api호출
            getRewardPointIssueExhaust();
        };

        //검색조건에 따른 리스트 재조회
        const reloadList = async () => {
            //차트도 초기화
            await getRewardPointIssueExhaustChart();
        ...
        // 차트조회
        const getRewardPointIssueExhaustChart = async () => {
            try {
                // 차트 조회
                formData.schTy = 'C';
                await _getRewardPointIssueExhaust(formData).then(result => {
        ...
        //리스트 조회
        const getRewardPointIssueExhaust = async () => {
            try {
                // 리스트 조회
                formData.schTy = 'L';
                await _getRewardPointIssueExhaust(formData).then(result => {
        ...
        const periodSortGroupChange = () => {
            getRewardPointIssueExhaustChart();
        };
        const excelParams = {
            params: formData,
            url: '/common/api/v1/statics/reward-point-issue-exhaust/excel'
        };
