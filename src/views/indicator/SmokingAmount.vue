    ...
    <section>
        ...
        <div class="ui-chart-wrap mt-20">
            ...
            <div class="ui-chart-item mt-10">
                <chart-bar key="chart1" ref="chartBar" :data-sets="state.chart.datasets" :height="state.chart.height"
                    :labels="state.chart.label" :options="state.chart.options" />
            </div>
            <div class="ui-chart-item mt-1">
                <chart-bar key="chart2" ref="chartBar2" :data-sets="state.chart2.datasets" :height="state.chart2.height"
                    :labels="state.chart2.label" :options="state.chart2.options" />
            </div>
        </div>
...
import { _getSmokingAmount } from '@/api/indicator.js';
import ChartBar from '@/views/indicator/components/ChartBar.vue';
import SelectBox from '@/components/ui/SelectBox.vue';

import { toGridUnit } from '@/views/indicator/util/chart.js';
...
export default {
    ...
    setup() {
        ...
        const toLocaleStringKo = (param) => {
            // 세자리 콤마
            return param.value ? param.value.toLocaleString('ko') : 0;
        };
                ...
                options: {
                    ...
                    scales: {
                        y: {
                            ...
                            display: true,
                            position: 'left',
                            title: { display: true, text: '회원수' }
                        },
                        y1: {
                            type: 'linear',
                            display: true,
                            position: 'right',
                            title: { display: true, text: '개피' },
                            grid: {
        ...
        const chartInit = () => {
            if (state.chartList.length > 0) {
                ...
                state.chart.datasets = Object.keys(state.chartList[0].yAxis[0]).filter(keys => !state.chart.datalabels[keys]?.disabled).map((item, index) => {
                    return {
                ...
                state.chart2.label = state.chartList.map(item => item.xAxis);
                state.chart2.datasets = Object.keys(state.chartList[0].yAxis[0]).filter(keys => !state.chart2.datalabels[keys]?.disabled).map((item, index) => {
                    return {
                        label: initColum.value.find(label => label.field === item).headerName.valueOf(),
                        data: state.chartList.map(data => data.yAxis[0][item]).valueOf(),
                        type: state.chart2.datalabels[item]?.type ?? 'bar',
                        order: state.chart2.datalabels[item]?.order ?? 1,
                        yAxisID: state.chart2.datalabels[item]?.yAxisID ?? 'y'
                    };
                });
        ...
        // 차트조회
        const getSmokingAmountChart = async () => {
            try {
                // 차트 조회
                formData.schTy = 'C';
                await _getSmokingAmount(formData).then(result => {
        ...
        const periodSortGroupChange = () => {
            getSmokingAmountChart();
        };
        const excelParams = {
            params: formData,
            url: '/common/api/v1/statics/smoking-amount/excel'
        };
