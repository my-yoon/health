...
import { _getMauKeepRate } from '@/api/indicator.js';
...
export default {
    ...
    setup() {
        ...
        const initColum = ref([
            ...
            {
                headerName: '당월 MAU(전월 신규 가입자 중)',
                flex: 1,
                field: 'man_cnt',
                valueFormatter: toLocaleStringKo
            },
            {
                headerName: '전월 신규 가입자',
                flex: 1,
                field: 'mbr_cnt',
                valueFormatter: toLocaleStringKo
            },
        ...
        // 차트조회
        const getMauKeepRateChart = async () => {
            try {
                // 차트 조회
                formData.schTy = 'C';
                await _getMauKeepRate(formData).then(result => {
        ...
        const excelParams = {
            params: formData,
            url: '/common/api/v1/statics/mau-keep-rate/excel'
        };
