<template>
    <section>
        ...
        <div class="ui-chart-wrap mt-20">
            ...
            <div v-for="(item, index) in state.barChartList" :key="index" class="ui-chart-item mt-2">
                <chart-bar ref="chartBar" :data-sets="item.datasets" :labels="item.label" :options="item.options"
                    height="200"></chart-bar>
            </div>
        </div>
...
import { _getAverageBodyComposition } from '@/api/indicator.js';
...
export default {
    ...
    setup() {
        ...
        const initColum = ref([
            ...
            {
                headerName: '평균 내장지방지수',
                flex: 1,
                field: 'visceraFatLevel',
                unit: 'level'
            },
            {
                headerName: '평균 근육량',
        ...
        const state = reactive({
            ...
            excelDown: {
                isReason: false,
                apiUrl: '/common/api/v1/statics/average-body-composition/excel'
            },
        ...
        const onChangedPage = (num) => {
            ...
            //api호출
            getAverageBodyComposition();
        };

        //검색조건에 따른 리스트 재조회
        const reloadList = async () => {
            //차트도 초기화
            await getAverageBodyCompositionChart();
        ...
        const chartInit = () => {
            if (state.chartList.length > 0) {
                ...
                state.chart.datasets = Object.keys(state.chartList[0].yAxis[0]).filter(keys => state.chart.datalabels[keys]).map((item, index) => {
                    return {
                        label: state.chart.datalabels[item]?.headerName.valueOf(),
                        data: state.chartList.map(data => data.yAxis[0][item]).valueOf(),
                        type: state.chart.datalabels[item]?.type,
                        order: state.chart.datalabels[item]?.order ?? 1,
                        yAxisID: state.chart.datalabels[item]?.yAxisID ?? 'y'
                    };
                });

                const barChartSet = (item) => {
                    return {
                        label: [],
                        datasets: [],
                        options: {
                ...
                const barChart = ['bodyMassIndex', 'basalMetabolicRate', 'waistCircumference', 'visceraFatLevel', 'bodyWaterMass'];

                state.barChartList = [];
                barChart.forEach((field, index) => {
                    const colum = initColum.value.find(item => item.field === field);
                    let charSet = barChartSet(colum);

                    charSet.label = state.chartList.map(item => item.xAxis);
                    charSet.datasets = [{
                        label: colum.headerName.valueOf(),
                        data: state.chartList.map(item => item.yAxis[0][colum.field]).valueOf(),
                        backgroundColor: Object.values(chartColorSet)[index]
                    }];

                    state.barChartList.push(charSet);
                });
            }
        };

        // 차트조회
        const getAverageBodyCompositionChart = async () => {
            try {
                // 차트 조회
                formData.schTy = 'C';
                await _getAverageBodyComposition(formData).then(result => {
        ...
        //리스트 조회
        const getAverageBodyComposition = async () => {
            try {
                // 리스트 조회
                formData.schTy = 'L';
                await _getAverageBodyComposition(formData).then(result => {
                    // state.list = result.data.data.list;
                    state.list = result.data.data.list.filter((d, i) => i + 1 > pager.offset && i + 1 <= pager.offset + pager.size);
        ...
        const periodSortGroupChange = () => {
            getAverageBodyCompositionChart();
        };
        const excelParams = {
            params: formData,
            url: '/common/api/v1/statics/average-body-composition/excel'
        };
        return {
            //공통 사용
            $Modal,
