...
import { _getEventPresentSituation } from '@/api/indicator.js';
...
export default {
    ...
    setup() {
        ...
        const onChangedPage = (num) => {
            ...
            //api호출
            getEventPresentSituation();
        };

        //검색조건에 따른 리스트 재조회
        const reloadList = async () => {
            //차트도 초기화
            await getEventPresentSituationChart();
        ...
        // 차트조회
        const getEventPresentSituationChart = async () => {
            try {
                // 차트 조회
                formData.schTy = 'C';
                await _getEventPresentSituation(formData).then(result => {
        ...
        //리스트 조회
        const getEventPresentSituation = async () => {
            try {
                // 리스트 조회
                formData.schTy = 'L';
                await _getEventPresentSituation(formData).then(result => {
        ...
        const periodSortGroupChange = () => {
            getEventPresentSituationChart();
        };

        const excelParams = {
            params: formData,
            url: '/common/api/v1/statics/event-present-situation/excel'
        };
