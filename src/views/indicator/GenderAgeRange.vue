            ...
            <div v-if="state.openMore" class="ui-data-filter-more">
                <div class="form-item">
                    ...
                    <div class="item">
                        <label>OS구분</label>
                        <span class="input">
                            <RadioList v-model="formData.osType" :list="state.osTypeList" name="osType"></RadioList>
                        </span>
                    </div>
                </div>
            </div>
...
import { _getGenderAgeRange } from '@/api/indicator.js';
...
import { chartColorSet, toGridUnit, toLocaleStringKo } from '@/views/indicator/util/chart.js';
...
export default {
    ...
    setup() {
        ...
        const initColum = ref([
            { headerName: '연령', field: 'age_nm' },
            {
                headerName: '여자',
                children: [
                    { headerName: '고객수', field: 'age_fsx_cnt', valueFormatter: toLocaleStringKo },
                    { headerName: '전체대비비율', field: 'tot_fsx_rat', unit: '%', valueFormatter: toGridUnit },
                    { headerName: '연령대비비율', field: 'age_fsx_rat', unit: '%', valueFormatter: toGridUnit }
                ]
            },
            {
                headerName: '남자',
                children: [
                    { headerName: '고객수', field: 'age_msx_cnt', valueFormatter: toLocaleStringKo },
                    { headerName: '전체대비비율', field: 'tot_msx_rat', unit: '%', valueFormatter: toGridUnit },
                    { headerName: '연령대비비율', field: 'age_max_rat', unit: '%', valueFormatter: toGridUnit }
                ]
            }
        ]);
        ...
        const chartInit = () => {
            if (state.chartList.length > 0) {
                const dataColumSet = {
                    age_fsx_cnt: {
                        label: '여성',
                        backgroundColor: chartColorSet.red
                    },
                    age_msx_cnt: {
                        label: '남성',
                        backgroundColor: chartColorSet.blue
                    }
                };

                state.chart.label = state.chartList.map(item => item.xAxis);
                state.chart.datasets = Object.keys(state.chartList[0].yAxis[0]).filter(keys => !!dataColumSet[keys]).map((item, index) => {
                    return {
                        label: dataColumSet[item].label.valueOf(),
                        data: state.chartList.map(data => data.yAxis[0][item]).valueOf(),
                        backgroundColor: dataColumSet[item].backgroundColor
                    };
                });
                //높이설정
        ...
        // 차트조회
        const getGenderAgeRangeChart = async () => {
            try {
                // 차트 조회
                formData.schTy = 'C';
                await _getGenderAgeRange(formData).then(result => {
        ...
        const periodSortGroupChange = () => {
            getGenderAgeRangeChart();
        };
        const excelParams = {
            params: formData,
            url: '/common/api/v1/statics/gender-age-range/excel'
        };
