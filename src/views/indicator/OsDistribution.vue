...
import { _getOsDistribution } from '@/api/indicator.js';
...
export default {
    ...
    setup() {
        ...
        const defalutInitColum = [
            { headerName: 'OS', width: 200, field: 'mbl_os', pinned: 'left' }
        ];
        const initColum = ref([]);
        ...
        const chartInit = () => {
            if (state.chartList.length > 0) {
                state.chart.label = state.chartList[0].yAxis.map(item => item.mbl_os);
                state.chart.datasets = [{
                    label: '',
                    data: state.chart.label.map((item, index) => {
                        return state.chartList.map(data => toNumber(data.yAxis.find(yAxis => yAxis.mbl_os === item).mbl_cnt)).reduce((a, b) => a + b);
                    })
                }];
                //높이설정
        ...
        // 차트조회
        const getOsDistributionChart = async () => {
            try {
                // 차트 조회
                formData.schTy = 'C';
                await _getOsDistribution(formData).then(result => {
                ...
                await _getOsDistribution(formData).then(result => {
                    ...
                    const _list = listAll.map((item, index) => {
                        ...
                        for (let i = keys.length - 1; i >= 0; i--) {
                            const key = keys[i] === 'mbl_os' ? keys[i] : dayJS(keys[i]).format('YYYY-MM-DD');
