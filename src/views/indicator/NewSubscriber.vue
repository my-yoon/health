...
import { _getCumulativeNewSubscriber } from '@/api/indicator.js';
...
export default {
    ...
    setup() {
        ...
        const initColum = ref([
            ...
            {
                headerName: '신규가입자(본인인증미완료)',
                flex: 1,
                field: 'occ_cnt',
                valueFormatter: toLocaleStringKo
            },
            {
                headerName: '신규가입자(본인인증완료)',
                flex: 1,
                field: 'oci_cnt',
                valueFormatter: toLocaleStringKo
            }
        ...
        const onChangedPage = (num) => {
            ...
            //api호출
            getCumulativeNewSubscriber();
        };

        //검색조건에 따른 리스트 재조회
        const reloadList = async () => {
            //차트도 초기화
            await getCumulativeNewSubscriberChart();
        ...
        // 차트조회
        const getCumulativeNewSubscriberChart = async () => {
            try {
                // 차트 조회
                formData.schTy = 'C';
                await _getCumulativeNewSubscriber(formData).then(result => {
        ...
        //리스트 조회
        const getCumulativeNewSubscriber = async () => {
            try {
                // 리스트 조회
                formData.schTy = 'L';
                await _getCumulativeNewSubscriber(formData).then(result => {
        ...
        const periodSortGroupChange = () => {
            getCumulativeNewSubscriberChart();
        };

        const excelParams = {
            params: formData,
            url: '/common/api/v1/statics/new-subscriber/excel'
        };
