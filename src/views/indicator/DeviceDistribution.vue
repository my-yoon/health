...
import { _getDeviceDistribution } from '@/api/indicator.js';
...
export default {
    ...
    setup() {
        ...
        const defalutInitColum = [
            { headerName: '디바이스', width: 200, field: 'mbl_dvc', pinned: 'left' }
        ];
        const initColum = ref([]);
        ...
        const reloadList = async () => {
            //차트도 초기화
            await getDeviceDistributionChart();
        ...
        const chartInit = () => {
            if (state.chartList.length > 0) {
                state.chart.label = state.chartList[0].yAxis.map(item => item.mbl_dvc);
                state.chart.datasets = [{
                    label: '',
                    data: state.chart.label.map((item, index) => {
                        return state.chartList.map(data => toNumber(data.yAxis.find(yAxis => yAxis.mbl_dvc === item).dvc_cnt)).reduce((a, b) => a + b);
                    })
                }];
                // 높이 설정
        ...
        // 차트조회
        const getDeviceDistributionChart = async () => {
            try {
                // 차트 조회
                formData.schTy = 'C';
                await _getDeviceDistribution(formData).then(result => {
                ...
                await _getDeviceDistribution(formData).then(result => {
                    ...
                    const _list = listAll.map((item, index) => {
                        ...
                        for (let i = keys.length - 1; i >= 0; i--) {
                            const key = keys[i] === 'mbl_dvc' ? keys[i] : dayJS(keys[i]).format('YYYY-MM-DD');
        ...
        const periodSortGroupChange = () => {
            getDeviceDistributionChart();
        };
        const excelParams = {
            params: formData,
            url: '/common/api/v1/statics/device-distribution/excel'
        };
