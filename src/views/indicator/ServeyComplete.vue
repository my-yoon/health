...
import { _getQuestionComplete } from '@/api/indicator.js';
...
export default {
    ...
    setup() {
        ...
        const defalutInitColum = [
            { headerName: '설문', width: 200, field: 'ctgr_nm', pinned: 'left' }
        ];
        const initColum = ref([]);
        ...
        const reloadList = async () => {
            //차트도 초기화
            await getQuestionCompleteChart();
        ...
        const chartInit = () => {
            if (state.chartList.length > 0) {
                ...
                state.chart.datasets = [{
                    label: '',
                    data: state.chartList.map(item => item.yAxis[0].ctgr_cnt)
                }];
                //높이설정
        ...
        // 차트조회
        const getQuestionCompleteChart = async () => {
            try {
                // 차트 조회
                formData.schTy = 'C';
                await _getQuestionComplete(formData).then(result => {
        ...
        //리스트 조회
        const getQuestionComplete = async () => {
            try {
                // 리스트 조회
                formData.schTy = 'L';
                await _getQuestionComplete(formData).then(result => {
        ...
        const periodSortGroupChange = () => {
            getQuestionCompleteChart();
        };
        const excelParams = {
            params: formData,
            url: '/common/api/v1/statics/question-complete/excel'
        };
        return {
            //공통 사용
            $Modal,
