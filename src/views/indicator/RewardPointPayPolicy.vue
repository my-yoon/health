...
import { _getRewardPointPayPolicy } from '@/api/indicator.js';
...
export default {
    ...
    setup() {
        ...
        const onChangedPage = (num) => {
            ...
            //api호출
            getRewardPointPayPolicy();
        };

        //검색조건에 따른 리스트 재조회
        const reloadList = async () => {
            //차트도 초기화
            await getRewardPointPayPolicyChart();
        ...
        // 차트조회
        const getRewardPointPayPolicyChart = async () => {
            try {
                // 차트 조회
                formData.schTy = 'C';
                await _getRewardPointPayPolicy(formData).then(result => {
        ...
        //리스트 조회
        const getRewardPointPayPolicy = async () => {
            try {
                // 리스트 조회
                formData.schTy = 'L';
                await _getRewardPointPayPolicy(formData).then(result => {
        ...
        const periodSortGroupChange = () => {
            getRewardPointPayPolicyChart();
        };

        const excelParams = {
            params: formData,
            url: '/common/api/v1/statics/reward-point-pay-policy/excel'
        };
