...
import { _getVocInquiryTransferCompany } from '@/api/indicator.js';
...
export default {
    ...
    setup() {
        ...
        const defalutInitColum = [
            { headerName: '이관사', width: 200, field: 'ntpr_nm', pinned: 'left' }
        ];
        const initColum = ref([]);
        ...
        const onChangedPage = (num) => {
            ...
            //api호출
            getVocInquiryTransferCompany();
        };

        //검색조건에 따른 리스트 재조회
        const reloadList = async () => {
            //차트도 초기화
            await getVocInquiryTransferCompanyChart();
        ...
        // 차트조회
        const getVocInquiryTransferCompanyChart = async () => {
            try {
                // 차트 조회
                formData.schTy = 'C';
                await _getVocInquiryTransferCompany(formData).then(result => {
        ...
        //리스트 조회
        const getVocInquiryTransferCompany = async () => {
            try {
                // 리스트 조회
                formData.schTy = 'L';
                await _getVocInquiryTransferCompany(formData).then(result => {
        ...
        const periodSortGroupChange = () => {
            getVocInquiryTransferCompanyChart();
        };
        const excelParams = {
            params: formData,
            url: '/common/api/v1/statics/voc-inquiry-transfer-company/excel'
        };
