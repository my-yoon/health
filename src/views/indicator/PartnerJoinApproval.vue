...
import { _getPartnerJoinApproval } from '@/api/indicator.js';
...
export default {
    ...
    setup() {
        ...
        const onChangedPage = (num) => {
            ...
            //api호출
            getPartnerJoinApproval();
        };

        //검색조건에 따른 리스트 재조회
        const reloadList = async () => {
            //차트도 초기화
            await getPartnerJoinApprovalChart();
        ...
        // 차트조회
        const getPartnerJoinApprovalChart = async () => {
            try {
                // 차트 조회
                formData.schTy = 'C';
                await _getPartnerJoinApproval(formData).then(result => {
        ...
        //리스트 조회
        const getPartnerJoinApproval = async () => {
            try {
                // 리스트 조회
                formData.schTy = 'L';
                await _getPartnerJoinApproval(formData).then(result => {
        ...
        const periodSortGroupChange = () => {
            getPartnerJoinApprovalChart();
        };
        const excelParams = {
            params: formData,
            url: '/common/api/v1/statics/partner-join-approval/excel'
        };
