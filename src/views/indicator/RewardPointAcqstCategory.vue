...
import { _getRewardPointAcqstCategory } from '@/api/indicator.js';
...
export default {
    ...
    setup() {
        ...
        const defalutInitColum = [
            { headerName: '카테고리', width: 250, field: 'ctgr_nm', pinned: 'left' }
        ];
        const initColum = ref([]);
        ...
        const onChangedPage = (num) => {
            ...
            //api호출
            getRewardPointAcqstCategory();
        };

        //검색조건에 따른 리스트 재조회
        const reloadList = async () => {
            //차트도 초기화
            await getRewardPointAcqstCategoryChart();
        ...
        const chartInit = () => {
            if (state.chartList.length > 0) {
                state.chart.label = state.chartList[0].yAxis.map(item => item.ctgr_nm);
                state.chart.datasets = [{
                    label: '',
                    data: state.chart.label.map((item, index) => {
                        return state.chartList.map(data => toNumber(data.yAxis.find(yAxis => yAxis.ctgr_nm === item).ctgr_pnt)).reduce((a, b) => a + b);
                    })
                }];
                //높이설정
        ...
        // 차트조회
        const getRewardPointAcqstCategoryChart = async () => {
            try {
                // 차트 조회
                formData.schTy = 'C';
                await _getRewardPointAcqstCategory(formData).then(result => {
        ...
        //리스트 조회
        const getRewardPointAcqstCategory = async () => {
            try {
                // 리스트 조회
                formData.schTy = 'L';
                await _getRewardPointAcqstCategory(formData).then(result => {
                    const listAll = result.data.data.list;
                    const _list = listAll.map((item, index) => {
                        const reversedItem = {};
                        const keys = Object.keys(item);
                        for (let i = keys.length - 1; i >= 0; i--) {
                            const key = keys[i] === 'ctgr_nm' ? keys[i] : dayJS(keys[i]).format('YYYY-MM-DD');
                            reversedItem[key] = listAll[index][keys[i]];
                        }
                        return reversedItem;
                    });
                    // console.log(_list);
        ...
        const periodSortGroupChange = () => {
            getRewardPointAcqstCategoryChart();
        };
        const excelParams = {
            params: formData,
            url: '/common/api/v1/statics/os-distributiony/excel'
        };
