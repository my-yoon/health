...
import { _getPartnerSubAccountApproval } from '@/api/indicator.js';
...
export default {
    ...
    setup() {
        ...
        const onChangedPage = (num) => {
            ...
            //api호출
            getPartnerSubAccountApproval();
        };

        //검색조건에 따른 리스트 재조회
        const reloadList = async () => {
            //차트도 초기화
            await getPartnerSubAccountApprovalChart();
        ...
        // 차트조회
        const getPartnerSubAccountApprovalChart = async () => {
            try {
                // 차트 조회
                formData.schTy = 'C';
                await _getPartnerSubAccountApproval(formData).then(result => {
        ...
        //리스트 조회
        const getPartnerSubAccountApproval = async () => {
            try {
                // 리스트 조회
                formData.schTy = 'L';
                await _getPartnerSubAccountApproval(formData).then(result => {
        ...
        const periodSortGroupChange = () => {
            getPartnerSubAccountApprovalChart();
        };

        const excelParams = {
            params: formData,
            url: '/common/api/v1/statics/partner-sub-account-approval/excel'
        };
