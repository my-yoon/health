...
import { _getHealthNews } from '@/api/indicator.js';
    ...
    setup() {
        ...
        const initColum = ref([
            ...
            },
            {
                headerName: '인기 건강뉴스',
                children: [
                    { headerName: '건강뉴스', field: 'news_nm' },
                    { headerName: '조회수', field: 'mbr_cnt', valueFormatter: toLocaleStringKo },
                    { headerName: '전체대비비율', field: 'mbr_rat', unit: '%', valueFormatter: toGridUnit }
                ]
            }
        ]);
        ...
        const state = reactive({
            ...
            excelDown: {
                isReason: false,
                apiUrl: '/common/api/v1/statics/health-news/excel'
            },
        ...
        const chartInit = () => {
            if (state.chartList.length > 0) {
                ...
                state.chart.datasets = [{
                    label: '조회 수',
                    data: state.chartList.map(data => data.yAxis[0].mbr_cnt)
                }];
                // 높이설정
        ...
        // 차트조회
        const getHealthNewsChart = async () => {
            try {
                // 차트 조회
                formData.schTy = 'C';
                await _getHealthNews(formData).then(result => {
        ...
        const excelParams = {
            params: formData,
            url: '/common/api/v1/statics/health-news/excel'
        };
