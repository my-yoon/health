...
import { _getCumulativeSubscriber } from '@/api/indicator.js';
import ChartBar from '@/views/indicator/components/ChartBar.vue';
import SelectBox from '@/components/ui/SelectBox.vue';
import { toLocaleStringKo, toRound } from '@/views/indicator/util/chart.js';
...
export default {
    ...
    setup() {
        ...
        const initColum = ref([
            ...
            {
                headerName: '누적가입자(본인인증미완료)',
                flex: 1,
                field: 'occ_cnt',
                valueFormatter: toLocaleStringKo
            },
            {
                headerName: '누적가입자(본인인증완료)',
                flex: 1,
                field: 'oci_cnt',
                valueFormatter: toLocaleStringKo
            },
        ...
        const onSelectPicker = (type, value, status) => {
            ...
            if (type === 'day') {
                ...
                /*if (dayJS().startOf('day').diff(state.endday, 'day') < 1) {
                    $Modal.alert({
                        message: '설정하신 기간에서 당일이후 데이터는 제외합니다.',
                        buttonText: {ok: '확인'}
                    });
                }*/
            }

            /*const dates = removeToDay(dayJS, {startday: state.startday, endday: state.endday})
            state.startday = dates.startday
            state.endday = dates.endday*/
        };
        ...
        const onChangedPage = (num) => {
            ...
            //api호출
            getCumulativeSubscriber();
        };

        //검색조건에 따른 리스트 재조회
        const reloadList = async () => {
            //차트도 초기화
            await getCumulativeSubscriberChart();
            ...
            if (state.chartList.length > 0) {
                ...
                state.chart.datasets = Object.keys(state.chartList[0].yAxis[0]).map((item, index) => {
                    return {
                        ...
                        type: state.chart.datalabels[item]?.type ?? 'line',
        ...
        // 차트조회
        const getCumulativeSubscriberChart = async () => {
            try {
                // 차트 조회
                let params = _.clone(formData);
                params.schTy = 'C';
                await _getCumulativeSubscriber(params).then(result => {
        ...
        //리스트 조회
        const getCumulativeSubscriber = async () => {
            try {
                // 리스트 조회
                let params = _.clone(formData);
                params.schTy = 'L';
        ...
        const periodSortGroupChange = () => {
            getCumulativeSubscriberChart();
        };
        const excelParams = {
            params: formData,
            url: '/common/api/v1/statics/cumulative-subscriber/excel'
        };
