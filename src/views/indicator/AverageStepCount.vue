...
import { _getAverageStepCount } from '@/api/indicator.js';
...
export default {
    ...
    setup() {
        ...
        const defalutInitColum = [
            { headerName: '연령별', width: 200, field: 'step_nm', pinned: 'left' }
        ];
        const initColum = ref([]);
        ...
        const reloadList = async () => {
            //차트도 초기화
            await getAverageStepCountChart();
        ...
        const chartInit = () => {
            if (state.chartList.length > 0) {
                ...
                state.chart.datasets = [{
                    data: state.chartList.map(item => item.yAxis[0].step_cnt)
                }];
                // 높이설정
        ...
        // 차트조회
        const getAverageStepCountChart = async () => {
            try {
                // 차트 조회
                formData.schTy = 'C';
                await _getAverageStepCount(formData).then(result => {
        ...
        //리스트 조회
        const getAverageStepCount = async () => {
            try {
                // 리스트 조회
                formData.schTy = 'L';
                await _getAverageStepCount(formData).then(result => {
        ...
        const periodSortGroupChange = () => {
            getAverageStepCountChart();
        };
        const excelParams = {
            params: formData,
            url: '/common/api/v1/statics/average-step-count/excel'
        };
