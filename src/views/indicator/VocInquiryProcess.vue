...
import { _getVocInquiryProcess } from '@/api/indicator.js';
...
import { toGridUnit, toLocaleStringKo } from '@/views/indicator/util/chart';
...
export default {
    ...
    setup() {
        ...
        const onChangedPage = (num) => {
            ...
            //api호출
            getVocInquiryProcess();
        };

        //검색조건에 따른 리스트 재조회
        const reloadList = async () => {
            //차트도 초기화
            await getVocInquiryProcessChart();
        ...
        // 차트조회
        const getVocInquiryProcessChart = async () => {
            try {
                // 차트 조회
                formData.schTy = 'C';
                await _getVocInquiryProcess(formData).then(result => {
        ...
        //리스트 조회
        const getVocInquiryProcess = async () => {
            try {
                // 리스트 조회
                formData.schTy = 'L';
                await _getVocInquiryProcess(formData).then(result => {
        ...
        const periodSortGroupChange = () => {
            getVocInquiryProcessChart();
        };

        const excelParams = {
            params: formData,
            url: '/common/api/v1/statics/voc-inquiry-process/excel'
        };
