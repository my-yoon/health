...
import { _getFeeling } from '@/api/indicator.js';
...
export default {
    ...
    setup() {
        ...
        const defalutInitColum = [
            { headerName: '감정', width: 200, field: 'strs_nm', pinned: 'left' }
        ];
        const initColum = ref([]);
        ...
        const chartInit = () => {
            if (state.chartList.length > 0) {
                ...
                state.chart.datasets = [{
                    data: state.chartList.map(item => item.yAxis[0].strs_cnt),
                    backgroundColor: state.chartList.map(item => item.yAxis[0].strs_pcd === '01' ? chartColorSet.green2 : item.yAxis[0].strs_pcd === '02' ? chartColorSet.green : chartColorSet.red)
                }];
                // 높이 설정
        ...
        // 차트조회
        const getFeelingChart = async () => {
            try {
                // 차트 조회
                formData.schTy = 'C';
                await _getFeeling(formData).then(result => {
                ...
                await _getFeeling(formData).then(result => {
                    ...
                    const _list = listAll.map((item, index) => {
                        ...
                        for (let i = keys.length - 1; i >= 0; i--) {
                            const key = keys[i] === 'strs_nm' ? keys[i] : dayJS(keys[i]).format('YYYY-MM-DD');
        ...
        const excelParams = {
            params: formData,
            url: '/common/api/v1/statics/feeling/excel'
        };
        return {
            //공통 사용
            $Modal,
