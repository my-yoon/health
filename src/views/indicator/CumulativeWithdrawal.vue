...
import { _getCumulativeWithdrawal } from '@/api/indicator.js';
import ChartBar from '@/views/indicator/components/ChartBar.vue';
import SelectBox from '@/components/ui/SelectBox.vue';
import { toLocaleStringKo } from '@/views/indicator/util/chart.js';
...
export default {
    ...
    setup() {
        ...
        const initColum = ref([
            ...
            {
                headerName: '누적탈퇴자(자진탈퇴)',
                flex: 1,
                field: 'vwd_cnt',
                valueFormatter: toLocaleStringKo
            },
            {
                headerName: '누적탈퇴자(강제탈퇴)',
                flex: 1,
                field: 'fwd_cnt',
                valueFormatter: toLocaleStringKo
            },
            {
                headerName: '누적탈퇴자(유효기간만료탈퇴)',
                flex: 1,
                field: 'vpe_cnt',
                valueFormatter: toLocaleStringKo
            }
        ...
        const onChangedPage = (num) => {
            ...
            //api호출
            getCumulativeWithdrawal();
        };

        //검색조건에 따른 리스트 재조회
        const reloadList = async () => {
            //차트도 초기화
            await getCumulativeWithdrawalChart();
        ...
        // 차트조회
        const getCumulativeWithdrawalChart = async () => {
            try {
                // 차트 조회
                let params = _.clone(formData);
                params.schTy = 'C';
                await _getCumulativeWithdrawal(params).then(result => {
        ...
        //리스트 조회
        const getCumulativeWithdrawal = async () => {
            try {
                // 리스트 조회
                let params = _.clone(formData);
                params.schTy = 'L';
        ...
        const periodSortGroupChange = () => {
            getCumulativeWithdrawalChart();
        };
        const excelParams = {
            params: formData,
            url: '/common/api/v1/statics/cumulative-withdrawal/excel'
        };
