            ...
            <div v-if="state.openMore" class="ui-data-filter-more">
                <div class="form-item">
                    ...
                    <div class="item">
                        <label>유무선구분</label>
                        <span class="input">
                            <RadioList v-model="formData.vocTy" :list="state.vocTyList" name="vocTy"></RadioList>
                        </span>
                    </div>
                </div>
            </div>
...
import { _getVocInquiryType } from '@/api/indicator.js';
...
export default {
    ...
    setup() {
        ...
        const defalutInitColum = [
            { headerName: '문의유형', width: 200, field: 'qstn_ty', pinned: 'left' }
        ];
        const initColum = ref([]);
        ...
        const chartInit = () => {
            if (state.chartList.length > 0) {
                state.chart.label = state.chartList[0].yAxis.map(item => item.qstn_ty);
                state.chart.datasets = [{
                    label: '문의 건',
                    data: state.chart.label.map((item, index) => {
                        return state.chartList.map(data => toNumber(data.yAxis.find(yAxis => yAxis.qstn_ty === item).qstn_cnt)).reduce((a, b) => a + b);
                    })
                }];
                // 높이설정
        ...
        // 차트조회
        const getVocInquiryTypeChart = async () => {
            try {
                // 차트 조회
                formData.schTy = 'C';
                await _getVocInquiryType(formData).then(result => {
                ...
                await _getVocInquiryType(formData).then(result => {
                    ...
                    const _list = listAll.map((item, index) => {
                        ...
                        for (let i = keys.length - 1; i >= 0; i--) {
                            const key = keys[i] === 'qstn_ty' ? keys[i] : dayJS(keys[i]).format('YYYY-MM-DD');
        ...
        const periodSortGroupChange = () => {
            getVocInquiryTypeChart();
        };
        const excelParams = {
            params: formData,
            url: '/common/api/v1/statics/voc-inquiry-type/excel'
        };
        return {
            //공통 사용
            $Modal,
