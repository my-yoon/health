...
import { _getVocInquiryChargeCompanyType } from '@/api/indicator.js';
...
export default {
    ...
    setup() {
        ...
        const defalutInitColum = [
            { headerName: '담당사', width: 200, field: 'ntpr_nm', pinned: 'left' }
        ];
        const initColum = ref([]);
        ...
        const onChangedPage = (num) => {
            pager.current = num;
            getVocInquiryChargeCompanyType();
        };

        //검색조건에 따른 리스트 재조회
        const reloadList = async () => {
            //차트도 초기화
            await getVocInquiryChargeCompanyTypeChart();
        ...
        const chartInit = () => {
            if (state.chartList.length > 0) {
                state.chart.label = state.chartList[0].yAxis.map(item => item.ntpr_nm);
                state.chart.datasets = [{
                    label: '문의 건',
                    data: state.chart.label.map((item, index) => {
                        return state.chartList.map(data => toNumber(data.yAxis.find(yAxis => yAxis.ntpr_nm === item).ntpr_cnt)).reduce((a, b) => a + b);
                    })
                }];
                // 높이설정
        ...
        // 차트조회
        const getVocInquiryChargeCompanyTypeChart = async () => {
            try {
                // 차트 조회
                formData.schTy = 'C';
                await _getVocInquiryChargeCompanyType(formData).then(result => {
        ...
        //리스트 조회
        const getVocInquiryChargeCompanyType = async () => {
            try {
                // 리스트 조회
                formData.schTy = 'L';
                await _getVocInquiryChargeCompanyType(formData).then(result => {
                    ...
                    const _list = listAll.map((item, index) => {
                        ...
                        for (let i = keys.length - 1; i >= 0; i--) {
                            const key = keys[i] === 'ntpr_nm' ? keys[i] : dayJS(keys[i]).format('YYYY-MM-DD');
        ...
        const periodSortGroupChange = () => {
            getVocInquiryChargeCompanyTypeChart();
        };

        const excelParams = {
            params: formData,
            url: '/common/api/v1/statics/voc-inquiry-charge-company/excel'
        };
