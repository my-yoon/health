...
import { _getExercise } from '@/api/indicator.js';
...
export default {
    ...
    setup() {
        ...
        const defalutInitColum = [
            { headerName: '운동', width: 200, field: 'exercise_nm', pinned: 'left' }
        ];
        const initColum = ref([]);
        ...
        const chartInit = () => {
            if (state.chartList.length > 0) {
                ...
                state.chart.datasets = [{
                    data: state.chartList.map(item => item.yAxis[0].exercise_cnt)
                }];
                // 높이 설정
        ...
        // 차트조회
        const getExerciseChart = async () => {
            try {
                // 차트 조회
                formData.schTy = 'C';
                await _getExercise(formData).then(result => {
        ...
        const excelParams = {
            params: formData,
            url: '/common/api/v1/statics/exercise/excel'
        };
