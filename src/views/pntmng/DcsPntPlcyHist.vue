    ...
    <section class="s1">
        ...
        <div class="ui-data-filter">
            <div class="form-item">
                ...
                <dateSerch :dateTitle="'변경일자'" @onSelectDate="onSelectPicker" :setDay="state.setDay" ref="dateSearch" :pickerOnly="true"/>
        ...
        <div class="tbl-wrap">
            ...
            <columlist :columlists="tablelist_dcsPntPlcyHist_label" @checkColum="checkOptions" />
...
import { _getDcsPntPlcyHistList } from '@/api/pntmng.js';
import { useCommFunc } from '@/core/helper/common.js';
import { tablelist_dcsPntPlcyHist_label } from '@/data/table/pntmngtable.js';
...
export default {
    ...
    setup() {
        ...
        const state = reactive({
            ...
            tableColum_c: _.clone(tablelist_dcsPntPlcyHist_label),
            rowData: [],
            defaultColDef: {
                sortable: true,
                resizable: true,
        ...
        const exelParams = reactive({
            ...
            url: '/common/api/v1/point/policy/history/list/excel'
        });
        ...
        const getDcsPntplcyHistList = async () => {
            try {
                let {params, check} = await reqData();
                if (check) return;
                const response = await _getDcsPntPlcyHistList(params);
                if (response.code == 'OK') {
        ...
        const closeModal = (type, val) => {
            let status = popInfo.modalType;
            isShow.value = false;
            if (type == 'modalconfirm') {
                if (status == 'pntPlcySearch') {
                    searchData.pntPlcyCd = val.pntPlcyCd;
                    searchData.pntPlcyNm = val.pntPlcyNm;
                }
            }
        };
        ...
        // 컬럼 변경
        const resetTable = () => {
            const initColum = _.clone(tablelist_dcsPntPlcyHist_label);
        ...
        return {
            tablelist_dcsPntPlcyHist_label,
            searchShow,
            toggleSearch,
            state,
            popInfo,
