<template>
    <section class="s1">
            ...
            <div v-if="searchShow" class="ui-data-filter-more">
                ...
                <div class="form-item">
                    <div class="item">
                        ...
                        <span class="input">
                            <span class="dv">
                                <CommonBtn :init="'선택'" :grCdId="'215000'" :val="searchData.pntStCd" :delOption="['215007']"
        ...
        <div class="tbl-wrap">
            <div class="table-util flex space-between">
                <div class="btn-set-m flex">
                    <button type="button" class="btn btn-ss" @click="reqStatusCheck('cancelAlt');">취소요청</button>
                ...
                <div class="btn-set-m flex align-end">
                    <span class="table-total">조회결과 총 <strong>{{pager.totalCnt.toLocaleString()}}</strong>건</span>
            ...
            <columlist :columlists="tablelist_issHistMng_label" @checkColum="checkOptions" />
        ...
        <DefaultModal :isShow="isShow && popInfo.modalType == 'cancelAlt'" :modalTitle="popInfo.modalTitle" :buttonConfirm="popInfo.confirmNm" :buttonCancel="popInfo.cancelNm"
            @modalclose="closeModal">
            <template #modalcontent>
                <div>{{ popInfo.modalMessage }}</div>
            </template>
        </DefaultModal>
        <DefaultModal :isShow="isShow && popInfo.modalType == 'reqCancel'" :modalTitle="popInfo.modalTitle" :buttonConfirm="popInfo.confirmNm" :menuCode="state.menuCode" :buttonCancel="popInfo.cancelNm"
            @modalclose="closeModal">
            <template #modalcontent>
                <AprvListPop :menuCode="state.menuCode" @memberAprv="memberAprv"/>
            </template>
        </DefaultModal>
        <IssHistMngDetailPop v-if="isShow && popInfo.modalType == 'detail'" :pntNoSn="state.data.pntNoSn" @modalPlcyclose="closeModal"/>
        <AthrRsnPop v-if="isShow && popInfo.modalType == 'cancel'" :baseData="popInfo" @rsnData="closeModal"/>
    </section>
</template>
<script>
...
import { _getIssHistList, _setAthrAprvReq } from '@/api/pntmng.js';
...
import { tablelist_issHistMng_label } from '@/data/table/pntmngtable.js';
...
export default {
    components: {DefaultModal, DateSerch, IssHistMngDetailPop, AthrRsnPop, AprvListPop, CommonBtn },
    setup() {
        ...
        const state = reactive({
            setDay: '1month',
            tableColum_c: _.clone(tablelist_issHistMng_label),
            rowData: [],
            defaultColDef: {
                sortable: true,
                resizable: true,
        ...
        onMounted(() => {
            if (history.state.pntPlcyCd) {
                searchData.searchPntKey = 'pntPlcyCd';
                searchData.searchPntValue = history.state.pntPlcyCd;
            }
            //마스킹 권한 확인 후 리스트조회
        ...
        const showModal = (type) => {
            ...
            switch (type) {
                case 'cancelAlt' :
                    state.memberAprv = '';
                    popInfo.modalTitle = '발급취소요청';
                    popInfo.modalMessage = '선택하신 항목을 발급 취소요청 하시겠습니까?';
                    break;
                case 'reqCancel' :
                    popInfo.modalTitle = '발급취소요청';
                    ...
                    break;
                case 'cancel' :
                    popInfo.modalTitle = '발급취소요청 사유';
                    popInfo.modalMessage = '발급취소 사유를 등록해주세요.';
                    popInfo.confirmNm = '등록';
                    break;
            }
            isShow.value = true;
        };
        ...
        const closeModal = async (type, val) => {
            ...
            if (type == 'modalconfirm') {
                if (status == 'cancel') {
                    let data = await cancelReqData(val);
                    setAthrAprvReq(data);
                } else if (status == 'cancelAlt') {
                    showModal('reqCancel');
        ...
        const setAthrAprvReq = async (data) => {
            try {
                let response = await _setAthrAprvReq(data);
                if (response.data.code == 'OK') {
                    onChangedPage(pager.current);
                } else {
                    modal(response.data.message ?? '취소 요청이 정상적으로 수행되지 않았습니다.');
                }
            } catch (err) {
                modal(err);
            }
        ...
        const reqStatusCheck = (type) => {
            ...
            var showValue = true;
            let pntStCd = ['215001', '215002', '215006'];
            ...
            if (showValue) {
                ...
                if (nonMatch.length > 0) {
                    let issStatus = state.selectList[0].pntStCdNm;
                    let message = issStatus + ' 상태는 취소요청이 불가합니다.';
                    modal(message);
                } else {
        ...
        // 컬럼 변경
        const resetTable = () => {
            const initColum = _.clone(tablelist_issHistMng_label);
