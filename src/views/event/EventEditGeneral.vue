<template>
    <section>
                                    ...
                                    <div class="reg-group wp-100">
                                        <div class="reg-item">
                                            <template v-for="(item, index) in state.benefitList" :key="index">
                                                ...
                                                <!-- 일반 즉시, 당첨 -->
                                                <div v-if="!(formData.pcpType === 'NONE') && (formData.eventType === 'GENERAL')"
        ...
        <eventMainImg>
            <template #btns>
                <div class="btn-set-m flex">
                    ...
                    <button type="button" class="btn btn-ss" @click="onSelectFile('fileInputList', 'remove')">삭제</button>
                </div>
            </template>
            <template #file>
        ...
        <div class="btn-bottom-set flex justify-center">
            ...
            <button type="button" class="btn btn-sl posi" @click="edtiEvent(formData.eventType)">저장</button>
        </div>
...
import { _detailEvent, _editGeneralEvent } from '@/api/event.js';
import { _uploadFile } from '@/api/common.js';
export default {
    components: { DefaultModal, DateSerch, DateSingle, pushAgree, fileInput, PrdAdd, eventApprDefault, eventBenefit, fileInputAtnd, EventPointPop, EventClause, eventMainImg, eventBanner, eventClauseList },
    setup() {
        ...
        const editSet = () => {
            ...
            //일반 
            formData.eventFrcYn = state.eventGeneral.eventFrcYn;
            formData.eventFrcCount = state.eventGeneral.eventFrcCount;
            formData.eventPoint = state.eventGeneral.pointPlcySn;
            formData.pointPlcyCd = state.eventGeneral.pointPlcyCd;
            state.pointValue = state.eventGeneral.eventPoint;
            state.eventGeneral.eventProductYn === 'Y' ? state.eventProductYn = true : state.eventProductYn = false;
            formData.eventBtnType = state.eventGeneral.eventBtnType;
            formData.eventBtnName = state.eventGeneral.eventBtnName;
            formData.eventBtnLink = state.eventGeneral.eventBtnLink;
            imgInit();
        };
        const imgInit = () => {
            // 이미지 처리
        ...
        const editGeneralEvent = async () => {
            try {
                ...
                console.log(params);
                const response = await _editGeneralEvent(params);
        ...
        const onChangeFormData = (caseType, value, index) => {
            ...
            } else if (caseType === 'eventBtnType' && value === 'DISABLE') {
                formData.eventBtnName = '';
                formData.eventBtnLink = '';
            } else {
