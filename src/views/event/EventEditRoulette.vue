...
import { _detailEvent, _editRouletteEvent } from '@/api/event.js';
    ...
    setup() {
        ...
        onMounted(() => {
            if (route.query.eventSn) {
                ...
                detailEvent();
                // changeCount(formData.eventBnefType);
            }
        });
        //이벤트 등록시 확인 얼럿
        const editEvent = (type) => {
            if (validCheck()) {
                ...
                $Modal.confirm({
                    ...
                    .then(async (success) => {
                        for (let i = 0; i < 6; i++) {
                            if (formData['roulettePoint' + (i + 1)] === state.eventRoulette['roulettePoint' + (i + 1)]) {
                                formData['roulettePoint' + (i + 1)] = state.eventRoulette['pointPlcySn' + (i + 1)];
                            }
                        }
                        await editRouletteEvent(type);
                    })
                    .catch(error => {
                        console.log(error);
                    });
        ...
        const editSet = () => {
            ...
            formData.rouletteCnt = state.eventRoulette.rouletteCnt;

            //상품 처리
            if (state.productList !== null) {
                formData.prodlist = state.productList;
                state.productList_ = formData.prodlist;
                for (let i = 0; i < (state.eventRoulette.rouletteCnt); i++) {
                    if (state.eventRoulette['rouletteProduct' + (i + 1)] !== 0) {
                        let list = state.productList_;
                        let item = list[0];
                        console.log(list);
                        console.log(i, '상품 있음', item.productNm);
                        formData['rouletteProductNm' + (i + 1)] = item.productNm;
                        list.splice(0, 1);
                    } else {
                        console.log(i, ' 상품없음 ');
                        formData['rouletteProductNm' + (i + 1)] = '';
                    }
                }
            }

            //룰렛
            formData.immyPayBnef = state.eventRoulette.immyPayBnef; //즉시지급
            formData.pzwnPayBnef = state.eventRoulette.pzwnPayBnef; // 당첨후 지급 상품
            for (let i = 0; i < formData.rouletteCnt; i++) {
                formData['roulettePcpCnt' + (i + 1)] = state.eventRoulette['roulettePcpCnt' + (i + 1)];
                formData['roulettePoint' + (i + 1)] = state.eventRoulette['roulettePoint' + (i + 1)];
                formData['roulettePrbl' + (i + 1)] = state.eventRoulette['roulettePrbl' + (i + 1)];
                formData['rouletteProduct' + (i + 1)] = state.eventRoulette['rouletteProduct' + (i + 1)];
                formData['rouletteTax' + (i + 1)] = state.eventRoulette['rouletteTax' + (i + 1)];
                formData['rouletteProduct' + (i + 1)] = state.eventRoulette['rouletteProduct' + (i + 1)];
                formData['rouletteTax' + (i + 1)] = state.eventRoulette['rouletteTax' + (i + 1)];
                formData['rouletteTxt' + (i + 1)] = state.eventRoulette['rouletteTxt' + (i + 1)];
                formData['rouletteType' + (i + 1)] = state.eventRoulette['rouletteType' + (i + 1)];
            }
            imgInit();
        ...
        //이벤트 등록
        const editRouletteEvent = async (type) => {
            try {
                let paramsComm = {
                ...
                let paramsRoulette = {
                    ...
                    rouletteType1: Number(formData.rouletteType1),
                    ...
                    rouletteType2: Number(formData.rouletteType2),
                    ...
                    rouletteType3: Number(formData.rouletteType3),
                    ...
                    rouletteType4: Number(formData.rouletteType4),
                    ...
                    rouletteType5: Number(formData.rouletteType5),
                    ...
                    rouletteType6: Number(formData.rouletteType6),
                ...
                console.log(params);
                const response = await _editRouletteEvent(params);
                console.log(response);
        ...
        const onChangeFormData = (caseType, value, index) => {
            ...
            if (value === 'text' || value === 'point') {
                ...
                if (value == 0) {
                    ...
                    formData[`rouletteProduct${index + 1}`] = 0;
                    formData[`roulettePoint${index + 1}`] = 0;
                    formData[`roulettePcpCnt${index + 1}`] = 0;
