                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="reg-group">
                                        <div class="reg-item">
                                            <span class="radio" style="margin-left:0"
                                                v-for="(item, index) in state.eventType" :key="index">
                                                <template v-if="formData.eventType === item.value">
...
import { _detailEvent, _editQuizEvent } from '@/api/event.js';
import { _uploadFile } from '@/api/common.js';
export default {
    components: { DefaultModal, DateSerch, pushAgree, DateSingle, eventClauseList, fileInput, PrdAdd, eventApprDefault, eventBenefit, eventApprQuiz, fileInputAtnd, EventPointPop, EventClause, eventMainImg, eventBanner },
    setup() {
        ...
        const validCheck = (type) => {
            ...
            for (const item of target) {
                ...
                if ((item === 'eventHintLink' && _.isEmpty(formData.eventHintLink)) && (!formData.eventHintLink.includes('https://') && !formData.eventHintLink.includes('http://'))) {
                    _.isEmpty(formData.eventHintLink) ? state.validState.message = '퀴즈 힌트 링크를 작성해 주세요' : state.validState.message = '링크는 http:// 또는 https:// 로 시작하여야 합니다.';
                    state.validState.errState = true;
                    break;
                }
        ...
        const editSet = () => {
            ...
            state.eventMaster.eventPzwnDt === '' ? state.setSingleDay = new Date().getTime() + 3600 * 1000 * 24 * 31 : state.setSingleDay = state.eventMaster.eventPzwnDt;
            dateSearch1.value.initDate();
            ...
            if (state.productList !== null) {
                formData.prodlist = state.productList;
                for (let i = 0; i < state.productList.length; i++) {
                    if (i === 0) {
                        state.addprodlist[0].checkbox = false;
                        state.addprodlist[0].tax = formData.prodlist[i].productTax === 'Y' ? true : false;
                        state.addprodlist[0].productTax = formData.prodlist[i].productTax;
                        state.addprodlist[0].productNm = formData.prodlist[i].productNm;
                        state.addprodlist[0].productCnt = formData.prodlist[i].productCnt;
                        state.addprodlist[0].text = formData.prodlist[i].text;
                    } else {
                        state.addprodlist.push({
                            checkbox: false,
                            tax: formData.prodlist[i].productTax === 'Y' ? true : false,
                            productTax: formData.prodlist[i].productTax,
                            productNm: formData.prodlist[i].productNm,
                            productCnt: formData.prodlist[i].productCnt,
                            text: formData.prodlist[i].text
                        });
                    }
                }
            }
            ...
            //퀴즈 
            formData.eventFrcYn = state.eventQuiz.eventFrcYn;
            formData.eventFrcCount = state.eventQuiz.eventFrcCount;
            formData.eventPoint = state.eventQuiz.pointPlcySn;
            formData.pointPlcyCd = state.eventQuiz.pointPlcyCd;
            state.pointValue = state.eventQuiz.eventPoint;
            state.eventQuiz.eventProductYn === 'Y' ? state.eventProductYn = true : state.eventProductYn = false;
            formData.eventAnswer = state.eventQuiz.eventAnswer;
            formData.eventHintLink = state.eventQuiz.eventHintLink;
            formData.eventHintType = state.eventQuiz.eventHintType;
            formData.eventHintTxt = state.eventQuiz.eventHintTxt;
            formData.eventQuiz1 = state.eventQuiz.eventQuiz1;
            formData.eventQuiz2 = state.eventQuiz.eventQuiz2;
            formData.eventQuiz3 = state.eventQuiz.eventQuiz3;
            formData.eventQuiz4 = state.eventQuiz.eventQuiz4;
            imgInit();
        };
        const imgInit = () => {
            // 이미지 처리
        ...
        const changeCount = (type, eventCase) => {
            resetParams();
            if (type === state.eventMaster.eventBnefType) {
                detailEvent();
            } else {
                if (type === 'AFTER') {
        ...
        //이벤트 등록
        const editQuizEvent = async (type) => {
            try {
                let paramsComm = {
                    eventSn: Number(formData.eventSn),
                ...
                console.log(params);
                const response = await _editQuizEvent(params);
            ...
            if (imgSize) {
                $Modal.simple({
                    ...
                    .then(async (success) => {
                        ...
                        state[state.imgNum] = [];
                        imgInit();
                        console.log(state.fileInputList, state[state.imgNum]);
        ...
        const onChangePoint = (value, type) => {
            ...
            console.log(formData.eventPoint, state.pointValue);
