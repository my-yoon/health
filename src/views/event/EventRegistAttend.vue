            ...
            <eventApprAttendance :formData="formData" @formDataChange="onChangeFormData" @chiocePoint="openPoint"
                ...
                <template #benefit>
                    <fileInputAtnd :fileInputList_Atnd="state.fileInputList_Atnd" :eventDayOption="state.eventDayOption"
                        @changefileList="atndOption" @chiocePoint="openPoint" :pointValue="state.pointValue"
                        @fileListDel="fileListDel">
                    </fileInputAtnd>
                </template>
                <template #pointError>
...
import { _regAtndEvent, _regAddEvent, _getEventClause } from '@/api/event.js';
import { _uploadFile } from '@/api/common.js';
export default {
    components: { DefaultModal, DateSerch, DateSingle, pushAgree, fileInput, eventApprAttendance, fileInputAtnd, EventPointPop, EventClause, eventMainImg, eventEtcImg, eventBanner, eventClauseList },
    setup() {
        ...
        const validCheck = () => {
            ...
            for (const item of target) {
                console.log(item, !formData[item], state.fileInputList[0].fileName[0].name);
        ...
        const fileListDel = (type, index) => {
            ...
            if (type === 'fileInputList') {
                ...
                console.log(state.addImgList);
                if (type.indexOf('chcEpsPayImg1_') === 0) {
                    state.addImgList[index].fileUploadtype1 = '';
                } else {
                    state.addImgList[index].fileUploadtype2 = '';
                }
        ...
        const atndOption = (caseType, index, type, value, imgfile, imgnum) => {
             ...
             *  type == id명,
             *  value == 작성된 value
             *  imgfile ==이미지체크를 위한 form data만
             *  imgnum == 이미지체크 하기 위해 배열번호
             */
            if (caseType === 'checkbox') {
                ...
                getImageInfo(imgfile[0], imageSizeCallback);
            } else {
