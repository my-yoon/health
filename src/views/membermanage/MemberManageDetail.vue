<template>
    <section>
                    ...
                    <table class="table reg">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td>
                                    <template v-if="state.memberInfoDetail.updDt !== null">
                                        <span v-if="state.memberInfoDetail.authSe">
                                            {{ state.memberInfoDetail.authSe }}
                                            ({{ state.memberInfoDetail.updrNm }})
                                        </span>
                                        <span v-else>
                                            {{ state.memberInfoDetail.updrNm }}
                                        </span>
                                        /
                                        {{ state.memberInfoDetail.updDt }}
                                    </template>
                                    <template v-else>
                                        -
                                    </template>
            ...
            <div class="ui-panels">
                <div class="ui-panel-item">
                    <div v-if="state.tabPannel === 'service'" class="ui-section">
                        <MemberService :menuInfo="menuInfo" :memberNum="state.detailNum" />
                    </div>
                    <div v-if="state.tabPannel === 'point'" class="ui-section">
                        <MemberPoint :menuInfo="menuInfo" :memberNum="state.detailNum" />
                    </div>
                    <div v-if="state.tabPannel === 'event'" class="ui-section">
                        <MemberEvent :menuInfo="menuInfo" :memberNum="state.detailNum" />
                    </div>
                    <div v-if="state.tabPannel === 'VOC'" class="ui-section">
                        <MemberVOC :menuInfo="menuInfo" :memberNum="state.detailNum" />
                    </div>
                    <div v-if="state.tabPannel === 'commerce'" class="ui-section">
                        커머스
                    </div>
                    <div v-if="state.tabPannel === 'agree'" class="ui-section">
                        <MemberAgree :menuInfo="menuInfo" :memberNum="state.detailNum" />
                    </div>
        ...
        <div class=" btn-bottom-set flex justify-center">
            <button class="btn btn-sl nega" type="button" @click="goToPage('/member/list')">
                목록으로
            </button>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="reg-group">
                                            ...
                                            <div class="reg-item" v-if="formData.fultRsnCd === '226007'">
                                                <commselect :className="'sm'" :initData="formData.frcdScsnTrgRsnTypCd"
                                                    :selectType="'searchCode.frcdScsnTrgRsnTypCd'"
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="form-item" style="margin-left:-20px">
                                            <dateSerch ref="dateSearch" :dateTitle="''" :pickerOnly="false" :nextTime="true"
            ...
            <template #modalcontent>
                <div>
                    <MemberHistory :mbrSn="state.detailNum" :mskgnRlsYn="state.mskgnRlsYn"
                        @closeModal="isHistoryShow = false" @openModal="isHistoryShow = true" />
                </div>
            </template>
        </DefaultModal>
        <!-- 개인정보 사유 -->
                            ...
                            <table class="table reg">
                                ...
                                <tbody v-if="state.modalMode === 'list' && (state.infoHistory)">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            {{ state.infoHistory.recFnm }}
                                            <!-- <template v-if="state.infoHistory.wtdAtflInfo !== null">
                                                {{ state.infoHistory.wtdAtflInfo?.recFnm }}
                                            </template>
                                            <template v-else>
                                                -
                                            </template> -->
...
export default {
    ...
    setup() {
        ...
        const formData = reactive({
            ...
            mbrEmad: computed(() => (state.mailType2 === '' || state.mailType2 === '103014') ? `${state.mailType1}` : `${state.mailType1}@${state.mailType2Name}`), //담당자이메일주소
            ...
            rqstpBln: null, // //요청자소속,
            rqstpRsn: null, // //요청사유,
            // wtdAtflSn: null, // //녹취파일순번, // 삭제 ( recFnm: 녹취 파일명 대체 )
            recFnm: '', // 녹취 파일명
            fultRsnCd: '226008', //이용중지사유코드(226001: 불건전이용)
            frcdScsnTrgRsnTypCd: '',
        ...
        const valueCheck = () => {
            ...
            console.log('state.memberInfoDetail.mbrHhpno >>> ', formData.mbrHhpnoFno, formData.mbrHhpnoMno, formData.mbrHhpnoBno);
        ...
        const addFaultMember = async () => {
            try {
                let params = {
                    ...
                    frcdScsnTrgRsnTypCd: formData.frcdScsnTrgRsnTypCd,
        ...
        const actionModal = (type, code, mode) => {
            if (type === 'infoChangeShow') {
                if (state.mailType2 !== '103014' && state.mailType1.includes('@')) {
                    return false;
                }
        ...
        const validCheck = (type) => {
            ...
            for (const item of target) {
                ...
                state.validState.message = (item === 'fultRsnCd') ? '요청코드를 입력해 주세요' : (item === 'fultRsn') ? '사유 설명 글을 입력해주세요' : (item === 'rqstpNm') ? '요청자를 입력해주세요' : (item === 'rqstpRsn') ? '요청자사유를 입력해 주세요' : '녹취파일명을 입력해 주세요';
        ...
        const selectedOptions = (valueCode, valueName, type) => {
            ...
            if (type === 'rprsTlnoFno' || type === 'mailType2') {
                if (type === 'mailType2') {
                    console.log('mail type', valueCode, valueName);
                    state.mailType1 = '';
