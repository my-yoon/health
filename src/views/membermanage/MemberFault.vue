<template>
    <section>
        ...
        <div class="ui-data-filter">
            <div class="form-item">
                <dateSerch :dateTitle="'기간조회'" @onSelectDate="onSelectPicker" :pickerOnly="false" :setDay="state.setDay"
                    ref="dateSearch"
                    :selectOptions="[{ label: '이용중지일자', value: 'fultApnmDt' }, { label: '가입일자', value: 'jnDate' }, { label: '수정일자', value: 'utDate' }]" />
                ...
                <div class="form-item">
                    <div class="item">
                        ...
                        <span class="input">
                            <span class="dv">
                                <!-- 검색조건에 없음 -->
                                <commselect :selectType="'searchCode.agelist'" :initData="state.memberAge"
                    ...
                    <div class="item">
                        ...
                        <span class="input">
                            <span class="dv">
                                <span class="radio">
                                    <input id="service01" name="service109000" type="radio" v-model="formData.mbrSexCd"
                                        value="">
        ...
        <div class="tbl-wrap">
            ...
            <div v-else>
                <AgGridVue :columnDefs="state.tableColum_c" :rowData="state.faultList" :defaultColDef="state.defaultColDef" @grid-ready="onGridReady" @cell-clicked="onCellClicked" class="ag-theme-alpine" :domLayout="state.domLayout" style="width:100%">
                </AgGridVue>
...
<script>
import { getCurrentInstance, computed, reactive, inject, onMounted, ref, onUnmounted, watch } from 'vue';
...
import { _getFaultMember } from '@/api/memberinfo.js';
export default {
    ...
    setup() {
        ...
        const initColum = ref([
            ...
            { headerName: '개인이메일', field: 'mbrEmad', maxWidth: 100 },
            {
                ...
                field: 'fultApnmDt',
                valueGetter: (params) => {
                    return dayJS(params.data.fultApnmDt).format('YYYY-MM-DD');
                }
            },
            { headerName: '이용중지사유', field: 'fultRsn' },
            { headerName: '이용중지등록자', field: 'admnNm' },
            {
                headerName: '가입일자',
                field: 'mbrJnDt',
                maxWidth: 150,
                valueGetter: (params) => {
                    return dayJS(params.data.mbrJnDt).format('YYYY-MM-DD');
                }
            }
        ]);
        /**
        ...
        const formData = reactive({
            ...
            mbrStCd: '104005',  // 회원상태코드 (104001: 정상)
        ...
        const initParams = () => {
            formData.inqType = '';
            formData.mbrStCd = '104005';
            formData.mbrSexCd = '';
            formData.aggpCd = '';
            formData.cnSercSe = '';
            formData.cnSercCts = '';
            // state.infAgrmAllYn = true;
        ...
        const onSelectPicker = (type, value, status) => {
            ...
            } else if (type === 'select') { // 가입일자 | 탈퇴일자 | 이용중지일자
        ...
        const getFaultMember = async () => {
            try {
                ...
                console.log(params.mbrSexCd, formData.mbrSexCd);
                const response = await _getFaultMember(params);
        ...
        const onCellClicked = (e) => {
            console.log(e.data);
            goToPage(`/member/fault/detail?mbrSn=${e.data.mbrSn}&&mskgnRlsYn=${(state.mskgnRlsYn ? 'Y' : 'N')}`);
        };
        ...
        const exelParams = reactive({
            params: {
                ...
                aggpCd: computed(() => formData.aggpCd),
