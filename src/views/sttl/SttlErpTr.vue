...
const btnComp = {
    template: '<button type="button" class="btn btn-ss" @click="cellClicked(this)">선택</button>',
    setup() {
        const cellClicked = (e) => {
            emitter.$emit('cellErpTrSelect', e.params);
        };
        return {
            cellClicked
        };
...
export default {
    ...
    setup(props) {
        ...
        onMounted(() => {
            if(!_.isEmpty(props.params.sttlBstd8Cts)){
                state.searchType = 'trCd';
                state.searchText = props.params.sttlBstd8Cts;
                getErpTrList();
            }
