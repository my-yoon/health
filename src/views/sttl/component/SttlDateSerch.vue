...
export default {
    ...
    setup(props) {
        ...
        const setFromDateD = (value) => {
            ...
            if(_.isDate(value)){
                ...
                switch(state.dayType){
                    case 'year':
                        returnValue = dayJS().add(-1, 'y');
                        break;
                    case '6month':
                        returnValue = dayJS().add(-6, 'M');
                        break;
                    case '1month':
                        returnValue = dayJS().add(-1, 'M');
                        break;
                    case 'week':
                        returnValue = dayJS().add(-1, 'w');
                        break;
                    case 'yesterday':
                        returnValue = dayJS().add(-1, 'd');
                        break;
                    case 'today':
                    default:
                        returnValue = dayJS();
        ...
        const onChangeDate = (type) => {
            ...
            if (type === 'today') {
                setFromDate(dayJS(start));
            } else if(type === 'yesterday'){
                setFromDate(dayJS(start).add(-1,'d'));
            } else if (type === 'week') {
                setFromDate(dayJS(start).add(-1,'w'));
            } else if (type === 'month') {
                setFromDate(dayJS(start).add(-1,'M'));
            } else if (type === '1month') {
                setFromDate(dayJS(start).add(-1,'M'));
            } else if (type === '3month') {
                setFromDate(dayJS(start).add(-3,'M'));
            } else if (type === '6month') {
