...
<div class="ui-section">
    <div class="ui-title-3">
        <p>※ 중분류 카테고리 영역은 해당 중분류가 존재하는 경우에만 표시되며, 대분류 삭제시 먼저 중분류 카테고리를 모두 삭제해주세요</p>
        <div class="abs">
    ...
    <div class="ui-content">
     ...
     <div v-for="(item, index) in state.classificationList" class="ui-category-item" :key="`mjcs-item-${item.classificationCode}-${index}`">
        <div class="ui-category-input">
            <label :for="`list-cate${index}`">대분류명</label>
...
<div class="btn-bottom-set flex justify-center">
    <button type="button" class="btn btn-sl posi"
        ...
        :disabled="state.classificationList.length < 2">
        저장
    </button>
</div>
...
import { clsfState, _getClassificationListWithSub, _setClassificationList, _getCanDeleteYn } from '@/api/healthnews.js';
...
export default {
    ...
    setup() {
        ...
        const validateName = (name, index) => {
            ...
            if (!noWhiteSpaceName || !name.length) {
                state.classificationList[index].classificationCodeName = '';
                simpleModal('대분류명을 입력하세요!');
            ...
            if (isDuplicatedName && noWhiteSpaceName) {
                state.classificationList[index].classificationCodeName = '';
                simpleModal('중복된 대분류명 입니다.');
                focusingNameInput(index);
                return false;
            }
            state.classificationList[index].classificationCodeName = noWhiteSpaceName;
            state.classificationList[index].classificationSectionCode = clsfState.codeGroup.mjcs.code;
        ...
        const validate = () => {
            ...
            if (invalidMaxLength) {
                openModal('대분류명은 최대 500자까지 가능합니다.');
            }
