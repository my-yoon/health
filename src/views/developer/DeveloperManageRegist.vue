                                    ...
                                    <div class="reg-group">
                                        <div class="reg-item">
                                            <input type="text"
                                                ...
                                                placeholder="영문4자리의 기업코드를 입력하여 주세요" v-model="formData.ntprUcd"
                                                :disabled="state.brnCheck === 'Y'">
                                        </div>
                                        <div class="reg-item">
                                            <button type="button" class="btn btn-slm"
...
import { _regDevRequest, _duplicateEidCheck, _duplicateCcdCheck, _duplicateEmailCheck, _duplicateCheck } from '@/api/developer.js';
...
export default {
    ...
    setup() {
        ...
        const formData = reactive({
            ...
            admnLvlId: 'F00', // 관리자레벨 Manager:C00,Associate:C01
        ...
        const resetFormData = () => {
            ...
            formData.admnLvlId = 'F00'; // 관리자레벨 Manager:C00,Associate:C01
        ...
        const modalOpen = async (modalName, params) => {
            ...
            if (modalName === 'isShowID') {
                ...
                if (state.brnCheck === 'Y' && state.membertype !== null) {
                    console.log('state.brnCheck', state.membertype.includes('111005'));
                    let joinType = '';
                    ...
                    if (state.membertype.includes('111005')) { // 디벨로퍼로 가입 되어 있을 때 ( 여기는 디벨로퍼 가입 페이지 )
                        msg = '디벨로퍼로 가입되어 있는 사업자등록번호 입니다. 동일 사업자등록번호로 중복가입이 불가 합니다.';
                    } else {
                ...
                $Modal.simple({
                    ...
                    .then(success => {
                        if (state.brnCheck === 'Y') {
                            if (state.membertype !== null && state.membertype.includes('111005')) {
                                state.brnType1 = '';
                                state.brnType2 = '';
                                state.brnType3 = '';
                                state.brnCheck = '';
        ...
        const regDevRequest = async () => {
            try {
                ...
                const response = await _regDevRequest(params);
                if (response.data.code === 'OK') {
                    await goToPage('developer/regist/list');
                }
                console.log(response);
            } catch (error) {
                console.log(error);
