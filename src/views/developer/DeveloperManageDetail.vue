<template>
    <section>
        ...
        <div class="btn-bottom-set flex justify-center">
            <button type="button" class="btn btn-sl nega" @click="goToPage('/developer/regist/list')">
                취소
            </button>
...
import { _getDetailDevRequest, _getApprDev, _rejoinDevRequest, _deleteDevRequest } from '@/api/developer.js';
...
export default {
    ...
    setup() {
        ...
        */
        ...
        // 상세
        const getDetailDevRequest = async () => {
            try {
                let params = {
                    mskgnRlsYn: state.mskgnRlsYn ? 'Y' : 'N'
                };
                const response = await _getDetailDevRequest(state.detailNum, params);
        ...
        const getApprDev = async () => {
            try {
                ...
                const response = await _getApprDev(state.detailNum, params);
        ...
        const rejoinDevRequest = async () => {
            try {
                ...
                const response = await _rejoinDevRequest(params);
                if (response.data.code === 'OK') {
                    goToPage('/developer/regist/list');
                }
            }
            catch (error) {
                console.log(error);
        ...
        //삭제
        const deleteDevRequest = async () => {
            try {
                let params = state.detailNum;
                const response = await _deleteDevRequest(params);
                if (response.data.code === 'OK') {
                    goToPage('/developeremanage');
                }
