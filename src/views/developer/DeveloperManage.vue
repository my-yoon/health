...
import { _getJoinDevRequest } from '@/api/developer.js';
export default {
    ...
    setup() {
        ...
        const exelParams = {
            params: formData,
            url: '/common/api/v1/developer/join-request/excel'
        };
        ...
        const onCellClicked = (e) => {
            console.log(e.data);
            goToPage(`/developermanagedetail?Sn=${e.data.sn}`);
        };
        // 삭제할 컬럼 선택
        ...
        const getJoinDevRequest = async () => {
            try {
                ...
                console.log(params);
                const response = await _getJoinDevRequest(params);
